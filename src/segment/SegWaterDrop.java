...
public class SegWaterDrop {
    ...
    public static ArrayList<BufferedImage> test(BufferedImage sImage) {
        SegWaterDrop model = new SegWaterDrop();
        ArrayList<BufferedImage> rlist = model.drop(sImage);
        return rlist;
    }
    ...
    public ArrayList<BufferedImage> drop(BufferedImage sourceImage) {
        ...
        for (int x = 0; x < width; x++) {
            for (int y = 0; y < height; y++) {
                if (ImageUtil.isBlack(sourceImage.getRGB(x, y))) {
                    histData[x]++;
                }
            }
        }
    ...
    private void doSplit(Point[] starts, Point[] ends) {
        ...
        for (int i = 0; i < ends.length; i++) {
            ...
            for (int x = start.x; x < end.x; x++) {
                if (ImageUtil.isBlack(sourceImage.getRGB(x, i))) {
